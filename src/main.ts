...
(function () {
  const myPlugin = (hook: any, _vm: any) => {
    ...
    hook.beforeEach(function (content: string) {
      (window as any).$docsify.formatUpdated = (time: number) => {
    ...
    hook.doneEach(function () {
      ...
      const modal = createModal(document, [...findSignOffSheet(document.body)]);
  ...
  (window as any).$docsify.plugins = [].concat((window as any).$docsify.plugins || [], myPlugin as any);
})();
